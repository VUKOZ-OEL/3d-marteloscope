...
# --- VSTUP / VÝSTUP ---
INPUT_PROJECT_JSON = r"data/pokojna_test_v2.json"
OUTPUT_PROJECT_JSON = r"data/pokojna_test_v2_2.json"

# --- LAS složka ---
# Defaultně se použije složka, kde leží INPUT_PROJECT_JSON.
# Pokud chceš jinou, nastav LAS_DIR_OVERRIDE na existující cestu.
LAS_DIR_OVERRIDE = r"C:/Users/krucek/OneDrive - vukoz.cz/DATA/_GS-LCR/SLP_Pokojna/PokojnaHora_3df"  # např. r"C:/data/las" nebo nech prázdné
...
def write_json(original_path: str, df: pd.DataFrame, output_path: str = None) -> None:
    ...
    for segment in data.get("segments", []):
        ...
        # ---- (1) Zachování Z-ové hodnoty v "position" ----
        z = 0.0
...
def load_project_json_light(file_path: str, las_dir_override: str = "") -> pd.DataFrame:
    ...
    for seg in data.get("segments", []):
        ...
        # apex base: prefer crownCenter, fallback position
        apex = None
        if isinstance(crown_center, list) and len(crown_center) >= 3:
            apex = crown_center
