...
response = AgGrid(
    ...
    update_mode=GridUpdateMode.VALUE_CHANGED,  # Změny hodnot v reálném čase
...
# Zpracování tlačítek
if isinstance(selected_rows, list) and len(selected_rows) > 0:  # Ověření, že jsou řádky vybrány
    selected_ids = [row["id"] for row in selected_rows]  # Extrahujeme ID vybraných řádků
    ...
    # Reset výběru (znovu vykreslení bez výběru)
    ...
    AgGrid(
        st.session_state.trees,
        gridOptions=grid_options,
        update_mode=GridUpdateMode.NO_UPDATE,  # Žádné přímé změny uživatelského výběru
        data_return_mode=DataReturnMode.FILTERED_AND_SORTED,
        theme="streamlit",
        enable_enterprise_modules=False,
        fit_columns_on_grid_load=True,
    ...
    st.warning("Žádné řádky nebyly vybrány.")
