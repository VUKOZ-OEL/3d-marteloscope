...
exclude = {
    "x", "y", "species", "speciesColorHex",
    "management_status", "managementColorHex",
...
with cA:
    species_sel = st.pills("**Filter Species:**", sp_all, default=sp_all, selection_mode="multi")
with cB:
    plot_container = st.container()

_, _, cC = st.columns([1, 1, 15])
with cC:
    mgmt_sel = st.pills("**Filter Management:**", mg_all, default=mg_all, selection_mode="multi")
...
# ------------------------------------------------------------
# XY RANGE BASED ON HEATMAP GRID (±1 m)
# ------------------------------------------------------------
buffer = 1.0

# Heatmap grid edges (X + Y)
x_edges = np.linspace(float(df0["x"].min()),
                      float(df0["x"].max()),
                      HEATMAP_NBINS + 1)

y_edges = np.linspace(float(df0["y"].min()),
                      float(df0["y"].max()),
                      HEATMAP_NBINS + 1)

# Final axes range tied to heatmap raster
...
def _panel(mask):
    ...
    for iy in range(Z.shape[0]):
        for ix in range(Z.shape[1]):
            val = Z[iy, ix]
            H[iy, ix] = f"{colorbar_title}: {val:.2f}" if colorbar_title != "Count" else f"Count: {val:.0f}"
...
if show_overlay:
    ...
    df_o[color_col] = df_o[color_col].where(df_o[color_col].astype(str).str.match(valid_hex), "#000000")
...
# ------------------------------------------------------------
# FIX AXES — all 3 panels must be square
# ------------------------------------------------------------
for c in (1, 2, 3):
    # X axis
    fig.update_xaxes(
    ...
    # Y axis — for second and third hide labels, BUT keep scaleanchor
    if c == 1:
        fig.update_yaxes(
            range=[ymin, ymax],
            title="y [m]",
        ...
        fig.update_yaxes(
            ...
            ticks="",
            showgrid=False,
            scaleanchor=f"x{c}",   # ← THIS FIXES THE RECTANGLE SHAPE
            scaleratio=1,
            row=1,
            col=c,
        )
