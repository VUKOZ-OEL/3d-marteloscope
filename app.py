...
# Set Page Title
st.set_page_config(
    page_title="3D-Marteloscope", page_icon=":material/nature_people:", layout="wide"
)

# Dash page with summary
dash_page = st.Page(
    "src/Dashboard.py", title="Info & controls", icon=":material/dashboard:"
)

# Tree & Crown stats
summary_page = st.Page("src/Summary.py", title="Summary", icon=":material/info:")
tree_page = st.Page(
    "src/Tree_stats.py", title="Tree Statistics", icon=":material/nature:"
)
canopy_page = st.Page(
    "src/Canopy_stats.py", title="Canopy Occupancy", icon=":material/forest:"
)
space_page = st.Page(
    "src/Space_comp.py", title="Space Competition", icon=":material/join:"
)
light_page = st.Page(
    "src/Light_comp.py", title="Sky View Factor", icon=":material/light_mode:"
)
...
att_page = st.Page(
    "src/Attributes.py", title="Attribute Table", icon=":material/data_table:"
)
analytics_page = st.Page(
    "src/Analytics.py", title="Analytics - Experimental", icon=":material/addchart:"
)
...
add_atts_page = st.Page(
    "src/Add_attributes_prj.py", title="Add attributes", icon=":material/list_alt_add:"
)
colors_page = st.Page(
    "src/Colors_settings.py", title="Colors", icon=":material/colors:"
)
# Sandbox
